...
export default class OpenListItem extends PureComponent {
            ...
            <CardItem activeOpacity={1} bordered={true} style={styles.cardItemStyle}>
                <View style={styles.headerViewStyle}>
                    ...
                    {buy ? <Text style={styles.buyStyle} >{strings.buy}</Text> : <Text style={styles.sellStyle} >{strings.sell}</Text>}
                        <Text style={styles.priceStyle}>{content.price}</Text>
                        <Text style={styles.contentStyle} >{content.pl}</Text>
                        {expanded ? <Icon type="Entypo" style={styles.chevronStyle} name="chevron-down" /> : 
                        <Icon type="Entypo" style={styles.chevronStyle} name="chevron-right" />}
                </View>
            </CardItem>
        )
    }
    _renderContent(content) {
        return (
            <CardItem bordered={true} style={styles.cardItemStyle}>
                ...
                <View style={styles.contentViewStyle}>
                    <Text style={styles.contentLabelStyle}>{strings.postedon}</Text>
                ...
                <View style={styles.contentViewStyle}>
                    <Text style={styles.contentLabelStyle}>{strings.currentprice}</Text>
                    <Text style={styles.contentTextStyle}>{content.current_price}</Text>
                </View>
            </CardItem>
        );
    }
...
const styles = StyleSheet.create({
    cardItemStyle: {
        ...
        borderColor:mainTheme.liveSignalBorderColor,
    },
    headerViewStyle: {
        flexDirection: 'row',
        flex:1,
    ...
    contentPairStyle: {
        flex: 1,
        color: mainTheme.liveSignalItemColor,
        fontWeight: 'bold',
    },
    buyStyle: {
        flex: 1,
